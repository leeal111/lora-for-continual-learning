...
################################################################################
# https://github.com/vlomonaco/core50/blob/master/scripts/python/data_loader.py
...
class CORE50(object):
    ...
    def __next__(self):
        ...
        train_y = np.asarray(train_y, dtype=np.int)  # np.float32
    ...
    def get_test_set(self):
        ...
        if self.preload:
            ...
            # test imgs
            test_x = self.get_batch_from_paths(test_paths).astype(
                np.float32
            )  # (44972, 128, 128, 3)

        test_y = self.labels[scen][run][-1]
        test_y = np.asarray(test_y, dtype=np.int)  # np.float32
...
if __name__ == "__main__":
    ...
    # and assuming the core50 location in ~/core50/128x128/  scenario="nicv2_79"
    dataset = CORE50(
        root="/home/datasets/cl-data/core50/core50_128x128", preload=True, scenario="ni"
    )  # scenario="ni" uses 8 domains

    # Get the fixed test set
    test_x, test_y = dataset.get_test_set()  # (44972, 128, 128, 3)
    print(test_y)

    # for i, test_batch in enumerate(test_x):  # test_x没有按增量任务划分数据，而是全部堆在一起
    #     pass
    ...
    for i, train_batch in enumerate(dataset):
        ...
        print(
            "train_x shape: {}, train_y shape: {}".format(train_x.shape, train_y.shape)
        )

        # 使用torch.unique()获取不重复元素并保持顺序
        unique_elements, _ = torch.unique(
            torch.tensor(train_y), sorted=True, return_inverse=True
        )
        print(unique_elements)  # 0~49

        # use the data
        pass

    print(f"Total number of the dataset:{sum}")  # 119894
    """
    ----------- batch 0 -------------
    train_x shape: (14991, 128, 128, 3), train_y shape: (14991,)

    ----------- batch 1 -------------
    train_x shape: (14995, 128, 128, 3), train_y shape: (14995,)

    ----------- batch 2 -------------
    train_x shape: (14986, 128, 128, 3), train_y shape: (14986,)

    ----------- batch 3 -------------
    train_x shape: (14994, 128, 128, 3), train_y shape: (14994,)

    ----------- batch 4 -------------
    train_x shape: (14989, 128, 128, 3), train_y shape: (14989,)

    ----------- batch 5 -------------
    train_x shape: (14989, 128, 128, 3), train_y shape: (14989,)

    ----------- batch 6 -------------
    train_x shape: (14966, 128, 128, 3), train_y shape: (14966,)

    ----------- batch 7 -------------
    train_x shape: (14984, 128, 128, 3), train_y shape: (14984,)
    """
