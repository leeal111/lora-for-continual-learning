...
from data import iGanFake, iCore50, iDomainNet
...
class DataManager(object):
    def __init__(self, dataset_name, shuffle, seed, init_cls, increment, args=None):
        ...
        while sum(self._increments) + increment < len(
            self._class_order
        ):  # len(self._class_order):400  increment:50
            self._increments.append(increment)
        offset = len(self._class_order) - sum(
            self._increments
        )  # self._increments: [50, 50, 50, 50, 50, 50, 50], offset:50
        if offset > 0:
    ...
    def get_dataset(self, indices, source, mode, appendent=None, ret_data=False):
        ...
        data, targets = [], []  # 当前train_data和test_datade样本是0~49升序排列的
        for idx in indices:
            ...
            # print(f"**** The number of samples in class [{idx}]:{class_targets.shape[0]}")
