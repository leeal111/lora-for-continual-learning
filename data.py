...
class iCore50(iData):
    ...
    def download_data(self):
        # datagen = CORE50(root=self.args["data_path"], scenario="ni")  # orig
        datagen = CORE50(root=self.args.data_path, preload=True, scenario="ni")
        ...
        train_x = np.concatenate(np.array(dataset_list)[:, 0])  # (119894, 128, 128, 3)
        train_y = np.concatenate(np.array(dataset_list)[:, 1])  # (119894,)
        self.train_data = train_x
        self.train_targets = train_y  # label本来是0~49，改成了0~399

        test_x, test_y = datagen.get_test_set()  # test_x:(44972, 128, 128, 3)
